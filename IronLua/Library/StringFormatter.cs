    ...
    static class StringFormatter
    {
        ...
        static void ParseSpecifier(FormatTag tag, string format, ref int i)
        {
            ...
            if (!formatSpecifiers.TryGetValue(format[i], out specifier))
                throw LuaRuntimeException.Create(null, ExceptionMessage.STRING_FORMAT_INVALID_OPTION, format);
        ...
        class FormatTag
        {
            ...
            public string Format(string value)
            {
                ...
                if (Specifier != FormatSpecifier.String)
                    throw LuaRuntimeException.Create(null, ExceptionMessage.INVOKE_BAD_ARGUMENT_GOT, Count + 1, "number", "string");
                FormatString(value);
                return sb.ToString();
            }
